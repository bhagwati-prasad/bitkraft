class Router {
  ...
  init() {
    ...
    console.log('[Router] Initialized on route:', this.currentRoute);
    console.log('[Router] Event listener attached to document');
  }

  handleClick(event) {
    console.log('[Router] Click detected on:', event.target);
    ...
    if (!link) {
      console.log('[Router] Not a link, ignoring');
      return;
    }
    ...
    if (!href || href.startsWith('http') || href.startsWith('#')) {
      console.log('[Router] External or hash link, allowing default');
      return;
    }
    
    console.log('[Router] Preventing default and navigating');
    event.preventDefault();
    event.stopPropagation();
